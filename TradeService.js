...
class TradeService {
  ...
  async processTrade(tradeSignal) {
    try {
      ...
      if (!tradeSignal.tok) {
        tradeSignal.tok = await this.marketDataService.marketDataProvider.resolveSymbolToToken(
          tradeSignal.sym,
          tradeSignal.stkPrc,
          tradeSignal.optTp,
          tradeSignal.expiry
  ...
  validateTradeSignal(tradeSignal) {
    ...
    // Generate trading symbol if not provided
    if (!tradeSignal.trdSym) {
      ...
      const strike = tradeSignal.stkPrc.replace('.00', '');

      tradeSignal.trdSym = `${tradeSignal.sym}${expiryYear}${expiryMonth}${expiryDay}${strike}${tradeSignal.optTp}`;
    }
